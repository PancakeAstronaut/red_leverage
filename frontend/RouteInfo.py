...
def info(start, endpoint):
    ...
    side_padding_l = Label(frame, bg='lightblue', text="                      ")
    side_padding_l.grid(column=0, row=0)
    side_padding_r = Label(frame, bg='lightblue', text="                      ")
    ...
    emp_lbl = Text(frame)
    emp_lbl.insert(END, "To: {}\nFrom: {}\nDuration: {}\nDistance: {}\n".format(str(addresses[0]), str(addresses[1]), str(durations[0]) + str(durations[1]), str(distances[0]) + str(distances[1])))
